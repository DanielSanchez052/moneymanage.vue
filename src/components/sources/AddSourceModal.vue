...
const emit = defineEmits(["source-created"])
...
const addSourceModalId = ref(null)
let addSourceModal; 
...
onMounted(() => {
    addSourceModal = new Modal(addSourceModalId.value) 
})

function handleAddSource(source, { resetForm }) {
    messages.value = []
    error.value = false
