<template>
<div ref="updateTransactionModalId" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-sm h-full md:h-auto">
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form @submit="handleUpdateTransaction" novalidate>
                <div class="">
                    <div class="mb-4">
                        <label for="ammount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Monto</label>
                        <Field type="number" focus name="ammount" id="ammount" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Escribe el monto de la transacción" />
...
const updateTransactionModalId = ref(null)
let updateTransactionModal;
...
onMounted(() => {
    updateTransactionModal = new Modal(updateTransactionModalId.value)
})

const handleUpdateTransaction = handleSubmit((transaction, { resetForm }) =>{
    messages.value = []
    error.value = false
    transaction.id = transactionId.value
    return TransactionService.UpdateTransaction(authStore.user.token, transaction).then(
        (res) => {
...
function showModal(transaction) {
    ...
    setFieldValue("ammount", transaction.ammount)
