      ...
      >
        ...
        <form @submit="handleAddTransaction" novalidate>
          <div class="">
            <div class="mb-4">
              ...
              <Field
                ...
                placeholder="Escribe el monto de la transacción"
              />
              <p
                name="ammount"
            ...
            <div class="mb-4">
              ...
              <div class="relative">
                ...
                <input
                  ...
                  @changeDate="
                    (event) => {
                      setValue(new Date(event.detail.date).toISOString());
                    }
                  "
                  @change="handleChange"
                  @blur="handleBlur"
            ...
            <div className="md:grid md:grid-cols-2 md:gap-2 mb-4">
              <div class="w-full">
                ...
                >
                  ...
                  <option :value="source.id" v-for="source in sources">
                    {{ source.name }}
                  </option>
                </Field>
                <p
                  name="sourceId"
                  class="text-center mt-2 text-sm text-red-600 dark:text-red-500"
                >
                  {{ errors.sourceId }}
                </p>
              </div>
              <div class="w-full">
                ...
                >
                  ...
                  <option :value="item.id" v-for="item in types">
                    {{ getTypeName(item.id) }}
                  </option>
                </Field>
                <p
            ...
            <section class="my-2 box-border rounded-sm border-2 p-2">
              ...
              <div class="overflow-y-auto max-h-72">
                <div
                  class="grid grid-rows-1 grid-flow-col gap-2 my-2"
                  v-for="(field, idx) in fields"
                  :key="field.key"
                >
                  <div>
                    <label
                      :for="`ext_prop[${idx}].displayName`"
                      class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                      >Nombre</label
                    >
                    <Field
                      type="text"
                      :name="`ext_prop[${idx}].displayName`"
                      class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                      placeholder="Escribe el monto de la transacción"
                    />
                    <ErrorMessage
                  ...
                  <div>
                    <label
                      :for="`ext_prop[${idx}].key`"
                      class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                      >Identificador</label
                    >
                    <Field
                      type="text"
                      :name="`ext_prop[${idx}].key`"
                      class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                      placeholder="Escribe el monto de la transacción"
                    />
                    <ErrorMessage
                  ...
                  <div>
                    <label
                      :for="`ext_prop[${idx}].value`"
                      class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                      >Valor</label
                    >
                    <Field
                      type="text"
                      :name="`ext_prop[${idx}].value`"
                      class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                      placeholder="Escribe el monto de la transacción"
                    />
                    <ErrorMessage
                  ...
                  <div class="flex items-end justify-center">
                    <button
                      @click="remove(idx)"
                      type="button"
                      class="focus:outline-none text-white bg-orange-700 hover:bg-orange-400 focus:ring-4 focus:bg-orange-100 font-medium rounded-lg text-sm px-5 py-2.5"
                    >
                ...
                <div class="flex items-center justify-end">
                  <button
                    type="button"
                    :class="button_active"
                    @click="push({ displayName: '', key: '', value: '' })"
                  >
                    <font-awesome-icon
                      icon="plus"
                      class="w-20"
          ...
          <div class="flex justify-end items-end w-full">
            <button
              class="w-full"
              :class="[isSubmitting ? button_disabled : button_active]"
...
const schema = yup.object().shape({
  ...
  transactionDate: yup
    .date()
    .required("La fecha de la transaccion es obligatoria"),
  ext_prop: yup
    .array()
    .of(
      yup.object().shape({
...
const button_active =
  "focus:outline-none text-white bg-indigo-600 hover:bg-indigo-800 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-900";
const button_disabled =
  "text-white bg-indigo-600 cursor-not-allowed font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-indigo-600";
...
const { errorMessage, handleBlur, handleChange, meta, setValue } =
  useField("transactionDate");

onMounted(() => {
  addTransactionModal = new Modal(addTransactionModalId.value);

  datePickerElement = document.getElementById("transactionDate");
...
const handleAddTransaction = handleSubmit((tansaction, { resetForm }) => {
  ...
  error.value = false;
  resetForm();
  datePickerElement.value = datePickerElement.defaultValue;
